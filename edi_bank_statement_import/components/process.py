...
class EDIExchangeBankStatementInput(Component):
    ...
    def _handle_result(self, statement_ids, filename):
        ...
        if len(statement_ids) == 1:
            self._handle_single_result(statement_ids[0])
            return True
        else:
            for statement_id in statement_ids:
                child_exchange = self.exchange_record.create({
                    'identifier': "%s_%s" % (self.exchange_record.identifier, statement_id),
                ...
                child_exchange._handle_single_result(statement_id)
                self.exchange_record.sudo().message_post(
                    body=_("Multiple Bank Statements created (child exchanges)") % statement.name
                )
            return True

    def _handle_single_result(self, statement_id):
